...
- **TLS Everywhere**:
  ...
  - NLB → Nodes: TLS re-encryption (port 8200, self-signed node certs)
...
Run once per environment before first deploy:

```bash
./scripts/generate-ca.sh vault-nonprod-test us-east-1
```

This creates a self-signed CA and stores it in AWS Secrets Manager.
...
│   └── generate-ca.sh           # Generate CA and store in Secrets Manager (run before first deploy)
├── jenkins/
...
- **NLB → Nodes**: TLS re-encryption (port 8200, self-signed node certs)
- **Node → Node**: Mutual TLS with self-signed CA (port 8201)
- **`api_addr`**: NLB domain (request forwarding goes through NLB)

The self-signed CA is generated out-of-band by `scripts/generate-ca.sh` and stored in AWS Secrets Manager (run once per environment before first deploy). Each node generates its own certificate at boot time, signed by this CA. The CA private key never enters Terraform state.
